...
- [ ] Create `product.schema.ts` with Product entity
  - [ ] Define Product schema (id, name, description, price, comparePrice, images, category, stock, storeId)
...
- [ ] Create `services/product.service.ts`
  ...
  - [ ] `updateProductStock(id, quantity)` - PATCH /products/:id/stock
...
- [ ] Create `hooks/useProducts.ts`
  ...
  - [ ] `useUpdateProductStock()` - Mutation for stock update
  - [ ] Implement optimistic updates with setQueryData
  - [ ] Handle cache invalidation properly
